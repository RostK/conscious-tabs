...
### E-1 and E-2 — 2026-09-24

Run by RostK against a real float, and **both passed**. That closes the sweep:
16 of 16.

They were deferred twice for the same reason — no competing video
Picture-in-Picture was set up — which is a poor reason to leave the two edges
that carry AC-34 and AC-35 unwitnessed. The blocker turned out to be tooling,
not difficulty: a synthetic video (canvas `captureStream`, no network, no site
to misbehave) makes both orders a two-minute check rather than a hunt for a
cooperative page.

What this actually settles is the part no test could reach. `float.test.ts`
drives the state machine behind AC-34 with 13 cases, but every one of them
_tells_ the module a float went away. Only a real eviction proves Chrome sends
the `pagehide` those cases assume, and that the focus heuristic reads it as an
eviction rather than a close the user asked for.
...
E-1 and E-2 remained deferred at that point on the same grounds as before: no
video PiP was set up. They were run on 2026-09-24 — see the section above.
...
- [x] **E-2 · our float evicts a video PiP, silently** — passed 2026-09-24
...
- [ ] **AC-20 · freshness while the anchor is backgrounded** _(the T-14 item)_
...
- [ ] **AC-21 · nothing running while idle** _(already met — confirm only if you want)_
...
- [x] **E-1 · a video PiP evicts our float** — passed 2026-09-24
...
## C. Keyboard and screen reader _(T-15 — same session, skip if you are short of time)_
